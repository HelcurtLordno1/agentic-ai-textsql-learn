...
class HybridRetriever:
    ...
    def retrieve(
        ...
        dense = self.dense.search(self.embed_query(query), fetch_k) if mode != "bm25" else []
        if mode == "hybrid":
            ranking = reciprocal_rank_fusion(
                {"bm25": bm25, "dense": dense}, {"bm25": 0.01, "dense": 1.0}
            )
        else:
            source = mode
        ...
        packed, used = pack_candidates(candidates, token_budget)
